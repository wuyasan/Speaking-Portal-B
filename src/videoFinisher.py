...
def generate_video(audioPath, framesPath, outputPath):
    ...
    command = "ffmpeg -r 30 -f image2 -s 1920x1080 -i "+str(framesPath)+"/f%06d.png -i "+str(
        audioPath) + " -vcodec libx264 -b 4M -c:a aac -strict -2 -pix_fmt yuv420p "+str(outputPath)+"/Vid_final.mp4 -y"
    subprocess.call(command, shell=True)
    emptyFolder(str(framesPath))
    return {
        "status": "success",
        "code": 200,
